...
class CityChoiceField(forms.ChoiceField):
    def __init__(self, province_id=None, *args, **kwargs):
        ...
        if province_id:
            get_cities = City.objects.filter(province=province_id)
            choices = []
...
class AddressForm(forms.ModelForm):
    def __init__(self, address=None,user=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if (address):
            self.fields['province'] = forms.ChoiceField(choices=Province.get_province_choices(), widget=forms.Select(attrs={'class' : 'form-control', 'id' : 'select-province', 'required': True}), initial=address.province.id)
            ...
            self.fields['province'] = forms.ChoiceField(choices=Province.get_province_choices(), widget=forms.Select(attrs={'class' : 'form-control', 'id' : 'select-province', 'required': True}), initial='')
    ...
    class Meta:
        model = Address
        fields = ['first_name','last_name','email', 'phone_number','province', 'city', 'district', 'village', 'alamat', 'post_code']
